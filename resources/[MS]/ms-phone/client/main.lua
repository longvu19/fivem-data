...
AddEventHandler('ms-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('ms-phone:server:SetPhoneAlerts', "phone")
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('ms-phone:server:SetPhoneAlerts', "phone", 0)
        ...
        if IsControlJustPressed(0, Config.OpenPhone) then
            if not PhoneData.isOpen then
                ...
                if not IsHandcuffed then
                    ...
                    MSCore.Functions.Notify("You can not do this right now.", "error")
                end
            end
        end
        Citizen.Wait(3)
...
function OpenPhone()
    MSCore.Functions.TriggerCallback('ms-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            MSCore.Functions.Notify("You do not have a phone", "error")
        end
    end)
end
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            SendNUIMessage({
                ...
                NotifyData = {
                    title = "Whatsapp",
                    content = "Received a message from "..IsNumberInContacts(SenderNumber).."!",
                    icon = "fab fa-whatsapp",
                    timeout = 3500,
                    color = "#25D366",
                },
            ...
            TriggerServerEvent('ms-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
        ...
        if PhoneData.isOpen then
            ...
            SendNUIMessage({
                ...
                NotifyData = {
                    title = "Whatsapp",
                    content = "Received a new message from "..IsNumberInContacts(SenderNumber).."!",
                    icon = "fab fa-whatsapp",
                    timeout = 3500,
                    color = "#25D366",
                },
...
RegisterNetEvent("ms-phone-new:client:BankNotify")
AddEventHandler("ms-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
        action = "Notification",
        NotifyData = {
            title = "Bank",
...
AddEventHandler('ms-phone:client:NewMailNotify', function(MailData)
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Mail",
                text = "Received a new email from "..MailData.sender,
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Mail",
                content = "Received a new email from "..MailData.sender,
                icon = "fas fa-envelope",
                timeout = 3500,
                color = "#ff002f",
            },
    ...
    TriggerServerEvent('ms-phone:server:SetPhoneAlerts', "mail")
end)
...
AddEventHandler('ms-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    if PhoneData.isOpen then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Adverts",
                text = "A new ad was posted by "..LastAd,
                icon = "fas fa-ad",
                color = "#ff8f1a",
                timeout = 2500,
            },
        ...
        SendNUIMessage({
            ...
            NotifyData = {
                title = "Adverts",
                content = "A new ad was posted by "..LastAd,
                icon = "fas fa-ad",
                timeout = 2500,
                color = "#ff8f1a",
            },
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('ms-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
                ...
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    ...
                    SetTimeout(2500, function()
                        SendNUIMessage({
                            ...
                            PhoneNotify = {
                                title = "Twitter",
                                text = "You can not mention yourself!",
                                icon = "fab fa-twitter",
                                color = "#1DA1F2",
                            },
                        })
...
AddEventHandler('ms-phone:client:TransferMoney', function(amount, newmoney)
    ...
    if PhoneData.isOpen then
        ...
        SendNUIMessage({ action = "Notification", NotifyData = { title = "Bank", content = "You have &dollar;"..amount.." available!", icon = "fas fa-university", timeout = 2500, color = nil, }, })
    end
end)
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('ms-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('ms-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    end)
end)
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        SendNUIMessage({ 
            ...
            PhoneNotify = { 
                title = "Phone",
                text = "You have no calls to answer...",
                icon = "fas fa-phone",
                color = "#e84118",
            }, 
        })
...
RegisterNetEvent('ms-phone:client:addPoliceAlert')
AddEventHandler('ms-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = MSCore.Functions.GetPlayerData().job
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    MSCore.Functions.Notify('Location marked on GPS for: '..data.alert.title)
...
AddEventHandler('ms-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('ms-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data)
    TriggerServerEvent('ms-lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerServerEvent('ms-lapraces:server:LeaveRace', data.RaceData)
end)
...
RegisterNUICallback('GetRaces', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('ms-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
end)

RegisterNUICallback('HasCreatedRace', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(data, cb)
    local InRace = exports['ms-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        local data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['ms-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('ms-lapraces:server:CreateLapRace', data.TrackName)
end)

RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-lapraces:server:GetRacingData', function(RaceData)
        local ped = GetPlayerPed(-1)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('ms-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            MSCore.Functions.Notify('You are too far from the race. The location was marked on your GPS.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['ms-lapraces']:IsInEditor())
    else
        cb(exports['ms-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(data, cb)
    MSCore.Functions.TriggerCallback('ms-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    MSCore.Functions.Notify("Your GPS has been marked for " .. data.HouseData.HouseData.adress .. "!", "success")
end)

RegisterNUICallback('RemoveKeyholder', function(data)
    TriggerServerEvent('ms-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    MSCore.Functions.Notify('GPS marcado!', 'success')
end)
...
RegisterNUICallback('GetTruckerData', function(data, cb)
    local TruckerMeta = MSCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
    local TierData = exports['ms-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
...
function DisableDisplayControlActions()
    ...
    -- DisableControlAction(0, 161, true) -- disable 5
    -- DisableControlAction(0, 162, true) -- disable 6
    -- DisableControlAction(0, 163, true) -- disable 7
    -- DisableControlAction(0, 164, true) -- disable 8
    -- DisableControlAction(0, 165, true) -- disable 9
    ...
    EnableControlAction(0, 249, true) -- enable N
    EnableControlAction(0, 76, true) -- enable handbrake / spacebar
end
