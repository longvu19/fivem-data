...
AddEventHandler('ms-radialmenu:client:setExtra', function(data)
    ...
    if veh ~= nil then
        ...
        if GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
            ...
            MSCore.Functions.Notify('Bạn không phải chủ xe!', 'error', 2500)
        end
    end
end)
    ...
    if not HasHarnass then
        ...
        if IsSeatFree then
            if kmh <= 100.0 then
                ...
                MSCore.Functions.Notify('Bạn đã chuyển sang ghế  '..data.title..'!')
            else
                MSCore.Functions.Notify('Giảm tốc độ trước khi chuyển ghế..')
            end
        else
            MSCore.Functions.Notify('Ghế đã có người ngồi..')
        end
    else
        MSCore.Functions.Notify('Bạn đang thắt dây an toàn, bạn không thể đổi ghế..', 'error')
    end
end)
