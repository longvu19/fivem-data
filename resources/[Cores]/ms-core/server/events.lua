-- Player joined
RegisterServerEvent("MSCore:PlayerJoined")
AddEventHandler('MSCore:PlayerJoined', function()
	local src = source
end)

AddEventHandler('playerDropped', function(reason) 
	local src = source
	print("Dropped: "..GetPlayerName(src))
	TriggerEvent("ms-log:server:CreateLog", "joinleave", "Dropped", "red", "**".. GetPlayerName(src) .. "** ("..GetPlayerIdentifiers(src)[1]..") left..")
	TriggerEvent("ms-log:server:sendLog", GetPlayerIdentifiers(src)[1], "joined", {})
	if reason ~= "Reconnecting" and src > 60000 then return false end
	if(src==nil or (MSCore.Players[src] == nil)) then return false end
	MSCore.Players[src].Functions.Save()
	MSCore.Players[src] = nil
end)

-- Checking everything before joining
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	deferrals.defer()
	local src = source
	deferrals.update("\nĐang kiểm tra tên...")
	local name = GetPlayerName(src)
	if name == nil then 
		MSCore.Functions.Kick(src, 'Không được để trống tên tài khoản Steam.', setKickReason, deferrals)
        CancelEvent()
        return false
	end
	if(string.match(name, "[*%%'=`\"]")) then
        MSCore.Functions.Kick(src, 'Tên tài khoản có kí tự ('..string.match(name, "[*%%'=`\"]")..') không được cho phép.', setKickReason, deferrals)
        CancelEvent()
        return false
	end
	if (string.match(name, "drop") or string.match(name, "table") or string.match(name, "database")) then
        MSCore.Functions.Kick(src, 'Tên tài khoản có chứa từ (drop / table / database) không được cho phép', setKickReason, deferrals)
        CancelEvent()
        return false
	end
	deferrals.update("\nĐang kiểm tra danh tính...")
    local identifiers = GetPlayerIdentifiers(src)
	local steamid = identifiers[1]
	local license = identifiers[2]
    if (MSConfig.IdentifierType == "steam" and (steamid:sub(1,6) == "steam:") == false) then
        MSCore.Functions.Kick(src, 'Bạn phải mở Steam để chơi.', setKickReason, deferrals)
        CancelEvent()
		return false
	elseif (MSConfig.IdentifierType == "license" and (steamid:sub(1,6) == "license:") == false) then
		MSCore.Functions.Kick(src, 'Không có giấy phép nào thuộc Social Club.', setKickReason, deferrals)
        CancelEvent()
		return false
    end
	deferrals.update("\nĐang kiểm tra trạng thái ban...")
    local isBanned, Reason = MSCore.Functions.IsPlayerBanned(src)
    if(isBanned) then
        MSCore.Functions.Kick(src, Reason, setKickReason, deferrals)
        CancelEvent()
        return false
    end
	deferrals.update("\nĐang kiểm tra whitelist...")
    if(not MSCore.Functions.IsWhitelisted(src)) then
        MSCore.Functions.Kick(src, 'Bạn không ở trong whitelist.', setKickReason, deferrals)
        CancelEvent()
        return false
    end
	deferrals.update("\nĐang kiểm tra trạng thái server...")
    if(MSCore.Config.Server.closed and not IsPlayerAceAllowed(src, "msadmin.join")) then
		MSCore.Functions.Kick(_source, 'Server đã đóng:\n'..MSCore.Config.Server.closedReason, setKickReason, deferrals)
        CancelEvent()
        return false
	end
	TriggerEvent("ms-log:server:CreateLog", "joinleave", "Queue", "orange", "**"..name .. "** ("..json.encode(GetPlayerIdentifiers(src))..") in queue..")
	TriggerEvent("ms-log:server:sendLog", GetPlayerIdentifiers(src)[1], "left", {})
	TriggerEvent("connectqueue:playerConnect", src, setKickReason, deferrals)
end)

RegisterServerEvent("MSCore:server:CloseServer")
AddEventHandler('MSCore:server:CloseServer', function(reason)
    local src = source
    local Player = MSCore.Functions.GetPlayer(src)

    if MSCore.Functions.HasPermission(source, "admin") or MSCore.Functions.HasPermission(source, "god") then 
        local reason = reason ~= nil and reason or "No specified motive..."
        MSCore.Config.Server.closed = true
        MSCore.Config.Server.closedReason = reason
        TriggerClientEvent("msadmin:client:SetServerStatus", -1, true)
	else
		MSCore.Functions.Kick(src, "Bạn không có quyền...", nil, nil)
    end
end)

RegisterServerEvent("MSCore:server:OpenServer")
AddEventHandler('MSCore:server:OpenServer', function()
    local src = source
    local Player = MSCore.Functions.GetPlayer(src)
    if MSCore.Functions.HasPermission(source, "admin") or MSCore.Functions.HasPermission(source, "god") then
        MSCore.Config.Server.closed = false
        TriggerClientEvent("msadmin:client:SetServerStatus", -1, false)
    else
        MSCore.Functions.Kick(src, "Bạn không có quyền...", nil, nil)
    end
end)

RegisterServerEvent("MSCore:UpdatePlayer")
AddEventHandler('MSCore:UpdatePlayer', function(data)
	local src = source
	local Player = MSCore.Functions.GetPlayer(src)
	
	if Player ~= nil then
		Player.PlayerData.position = data.position

		local newHunger = Player.PlayerData.metadata["hunger"] - 4.2
		local newThirst = Player.PlayerData.metadata["thirst"] - 3.8
		if newHunger <= 0 then newHunger = 0 end
		if newThirst <= 0 then newThirst = 0 end
		Player.Functions.SetMetaData("thirst", newThirst)
		Player.Functions.SetMetaData("hunger", newHunger)

		Player.Functions.AddMoney("bank", Player.PlayerData.job.payment)
		TriggerClientEvent('MSCore:Notify', src, "Lương về $"..Player.PlayerData.job.payment)
		TriggerClientEvent("hud:client:UpdateNeeds", src, newHunger, newThirst)

		Player.Functions.Save()
	end
end)

RegisterServerEvent("MSCore:UpdatePlayerPosition")
AddEventHandler("MSCore:UpdatePlayerPosition", function(position)
	local src = source
	local Player = MSCore.Functions.GetPlayer(src)
	if Player ~= nil then
		Player.PlayerData.position = position
	end
end)

RegisterServerEvent("MSCore:Server:TriggerCallback")
AddEventHandler('MSCore:Server:TriggerCallback', function(name, ...)
	local src = source
	MSCore.Functions.TriggerCallback(name, src, function(...)
		TriggerClientEvent("MSCore:Client:TriggerCallback", src, name, ...)
	end, ...)
end)

RegisterServerEvent("MSCore:Server:UseItem")
AddEventHandler('MSCore:Server:UseItem', function(item)
	local src = source
	local Player = MSCore.Functions.GetPlayer(src)
	if item ~= nil and item.amount > 0 then
		if MSCore.Functions.CanUseItem(item.name) then
			MSCore.Functions.UseItem(src, item)
		end
	end
end)

RegisterServerEvent("MSCore:Server:RemoveItem")
AddEventHandler('MSCore:Server:RemoveItem', function(itemName, amount, slot)
	local src = source
	local Player = MSCore.Functions.GetPlayer(src)
	Player.Functions.RemoveItem(itemName, amount, slot)
end)

RegisterServerEvent("MSCore:Server:AddItem")
AddEventHandler('MSCore:Server:AddItem', function(itemName, amount, slot, info)
	local src = source
	local Player = MSCore.Functions.GetPlayer(src)
	Player.Functions.AddItem(itemName, amount, slot, info)
end)

RegisterServerEvent('MSCore:Server:SetMetaData')
AddEventHandler('MSCore:Server:SetMetaData', function(meta, data)
    local src = source
	local Player = MSCore.Functions.GetPlayer(src)
	if meta == "hunger" or meta == "thirst" then
		if data > 100 then
			data = 100
		end
	end
	if Player ~= nil then 
		Player.Functions.SetMetaData(meta, data)
	end
	TriggerClientEvent("hud:client:UpdateNeeds", src, Player.PlayerData.metadata["hunger"], Player.PlayerData.metadata["thirst"])
end)

AddEventHandler('chatMessage', function(source, n, message)
	if string.sub(message, 1, 1) == "/" then
		local args = MSCore.Shared.SplitStr(message, " ")
		local command = string.gsub(args[1]:lower(), "/", "")
		CancelEvent()
		if MSCore.Commands.List[command] ~= nil then
			local Player = MSCore.Functions.GetPlayer(tonumber(source))
			if Player ~= nil then
				table.remove(args, 1)
				if (MSCore.Functions.HasPermission(source, "god") or MSCore.Functions.HasPermission(source, MSCore.Commands.List[command].permission)) then
					if (MSCore.Commands.List[command].argsrequired and #MSCore.Commands.List[command].arguments ~= 0 and args[#MSCore.Commands.List[command].arguments] == nil) then
					    TriggerClientEvent('chatMessage', source, "Hệ thống", "error", "Phải điền đầy đủ tham số")
					    local agus = ""
					    for name, help in pairs(MSCore.Commands.List[command].arguments) do
					    	agus = agus .. " ["..help.name.."]"
					    end
				        TriggerClientEvent('chatMessage', source, "/"..command, false, agus)
					else
						MSCore.Commands.List[command].callback(source, args)
					end
				else
					TriggerClientEvent('chatMessage', source, "Hệ thống", "error", "Không có quyền dùng lệnh này")
				end
			end
		end
	end
end)

RegisterServerEvent('MSCore:CallCommand')
AddEventHandler('MSCore:CallCommand', function(command, args)
	if MSCore.Commands.List[command] ~= nil then
		local Player = MSCore.Functions.GetPlayer(tonumber(source))
		if Player ~= nil then
			if (MSCore.Functions.HasPermission(source, "god")) or (MSCore.Functions.HasPermission(source, MSCore.Commands.List[command].permission)) or (MSCore.Commands.List[command].permission == Player.PlayerData.job.name) then
				if (MSCore.Commands.List[command].argsrequired and #MSCore.Commands.List[command].arguments ~= 0 and args[#MSCore.Commands.List[command].arguments] == nil) then
					TriggerClientEvent('chatMessage', source, "Hệ thống", "error", "Phải điền đầy đủ tham số")
					local agus = ""
					for name, help in pairs(MSCore.Commands.List[command].arguments) do
						agus = agus .. " ["..help.name.."]"
					end
					TriggerClientEvent('chatMessage', source, "/"..command, false, agus)
				else
					MSCore.Commands.List[command].callback(source, args)
				end
			else
				TriggerClientEvent('chatMessage', source, "Hệ thống", "error", "Không có quyền dùng lệnh này!")
			end
		end
	end
end)

RegisterServerEvent("MSCore:AddCommand")
AddEventHandler('MSCore:AddCommand', function(name, help, arguments, argsrequired, callback, persmission)
	MSCore.Commands.Add(name, help, arguments, argsrequired, callback, persmission)
end)

RegisterServerEvent("MSCore:ToggleDuty")
AddEventHandler('MSCore:ToggleDuty', function()
	local src = source
	local Player = MSCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.onduty then
		Player.Functions.SetJobDuty(false)
		TriggerClientEvent('MSCore:Notify', src, "Bạn đã ngoài giờ làm")
	else
		Player.Functions.SetJobDuty(true)
		TriggerClientEvent('MSCore:Notify', src, "Bạn đang trong giờ làm")
	end
	TriggerClientEvent("MSCore:Client:SetDuty", src, Player.PlayerData.job.onduty)
end)

Citizen.CreateThread(function()
	MSCore.Functions.ExecuteSql(true, "SELECT * FROM `permissions`", function(result)
		if result[1] ~= nil then
			for k, v in pairs(result) do
				MSCore.Config.Server.PermissionList[v.steam] = {
					steam = v.steam,
					license = v.license,
					permission = v.permission,
					optin = true,
				}
			end
		end
	end)
end)

MSCore.Functions.CreateCallback('MSCore:HasItem', function(source, cb, itemName)
	local retval = false
	local Player = MSCore.Functions.GetPlayer(source)
	if Player ~= nil then 
		if Player.Functions.GetItemByName(itemName) ~= nil then
			retval = true
		end
	end
	
	cb(retval)
end)	

RegisterServerEvent('MSCore:Command:CheckOwnedVehicle')
AddEventHandler('MSCore:Command:CheckOwnedVehicle', function(VehiclePlate)
	if VehiclePlate ~= nil then
		MSCore.Functions.ExecuteSql(false, "SELECT * FROM `player_vehicles` WHERE `plate` = '"..VehiclePlate.."'", function(result)
			if result[1] ~= nil then
				MSCore.Functions.ExecuteSql(false, "UPDATE `player_vehicles` SET `state` = '1' WHERE `citizenid` = '"..result[1].citizenid.."'")
				TriggerEvent('ms-garages:server:RemoveVehicle', result[1].citizenid, VehiclePlate)
			end
		end)
	end
end)