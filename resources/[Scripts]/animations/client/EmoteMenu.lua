...
mainMenu = NativeUI.CreateMenu("Hành động", "", menuPosition["x"], menuPosition["y"], Menuthing, Menuthing)
...
function AddEmoteMenu(menu)
    local submenu = _menuPool:AddSubMenu(menu, "Hành động", "", "", Menuthing, Menuthing)
    local dancemenu = _menuPool:AddSubMenu(submenu, "🕺 Nhảy", "", "", Menuthing, Menuthing)
    local propmenu = _menuPool:AddSubMenu(submenu, "📦 Hành động với vật dụng", "", "", Menuthing, Menuthing)
    --favmenu:AddItem(unbind2item)
    table.insert(EmoteTable, "🕺 Nhảy")
    table.insert(EmoteTable, "🕺 Nhảy")
...
function AddCancelEmote(menu)
    local newitem = NativeUI.CreateItem("Dừng hành động ", "~r~X~w~ Parar emote")
...
function AddWalkMenu(menu)
    local submenu = _menuPool:AddSubMenu(menu, "Dáng đi", "", "", Menuthing, Menuthing)

    walkreset = NativeUI.CreateItem("Bình thường (Reset)", "Default")
    submenu:AddItem(walkreset)
    table.insert(WalkTable, "Reset về mặc định")

    WalkInjured = NativeUI.CreateItem("Bị thương", "")
...
function AddFaceMenu(menu)
    local submenu = _menuPool:AddSubMenu(menu, "Tâm trạng", "", "", Menuthing, Menuthing)

    facereset = NativeUI.CreateItem("Bình thường (Reset)", "Reset về mặc định")
...
AddEventHandler('animations:client:ToggleMenu', function()
  if CanDoEmote then
    ...
    MSCore.Functions.Notify("Không thể thực hiện hành động lúc này", "error")
  end
end)
